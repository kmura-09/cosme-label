...
export function buildClaimPrompt(facts, opts = {}) {
  ...
  const craft = [
    ...
    compact ? "" : "悪い例: 「配合成分から選ぶ、毎日のケア。」「保湿成分とエモリエント成分を配合。」(何も伝わらない)",
  ...
  if (mode === "cosmetic") {
    rules.push(
      ...
      hints.length ? "この剤型で特に使える効能表現: " + (compact ? hints.slice(0, 8) : hints).join("／") : "",
    ...
    rules.push("表現の法規制は考慮しなくてよい。ただし事実 (成分表と候補) に無い事実を作らない。");
  }
  rules.push(
  ...
  const system = [...craft.filter(Boolean), ...rules.filter(Boolean), ...output].join("\n");
